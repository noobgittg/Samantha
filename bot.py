...
async def keep_alive():
    async with aiohttp.ClientSession() as session:
        while True:
            try:
                await session.get("https://adequate-quail-filterbotkn-818b00d4.koyeb.app/")  # Uses the imported URL from info.py
...
class Bot(Client):
    ...
    async def start(self):
        ...
        if SECONDDB_URI and free_dbSize<300: #if the primary db have less than 10MB left, use second DB.
