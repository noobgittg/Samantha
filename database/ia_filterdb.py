...
# 🧠 cache layer (redis-ready placeholder)
CACHE = {}
...
async def save_file(media):    
    ...
    try:    
        # 🔧 FIX: proper async duplicate check per DB
        existing = await saveMedia.find_one({"file_id": file_id})
        if existing:    
            logger.warning(f'{getattr(media, "file_name", "NO_FILE")} already exists in DB !')    
            return False, 0    
        ...
        try:    
            ...
            logger.warning(    
                f'{getattr(media, "file_name", "NO_FILE")} duplicate detected in DB'    
            )    
            return False, 0    
        else:    
